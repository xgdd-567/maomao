    ...
    document.getElementById('toggleFoot').addEventListener('click', function () {
      if (!expanded) {
        footEl.innerHTML = '<b>详细说明：</b>' +
          ...
          '2)「实际控制区」分为乌控（蓝）与俄控（红）两类，多边形来自 DeepStateMap.live 公开 API（2026-08-18 14:50 快照）。俄控含 2014 年占领的克里米亚与顿巴斯部分区域（该数据源以深色区分）。<br>' +
          '3)「争夺区」= DeepState 标注为「状态不明（Статус невідомий）」的地带，多为双方接触线附近的灰色地带，不代表精确前线。<br>' +
          '4) 争夺点与战况摘要逐条摘自 ISW 2026-08-17《Russian Offensive Campaign Assessment》报告原文（覆盖 8/16-17 战况，标题栏已列链接），报道链接为近三天（2026-08-16 ~ 08-19）真实公开来源（Reuters/AP、BBC、Kyiv Independent、Suspilne、Ukrainska Pravda、CNBC 等）。<br>' +
          '5) 战线每天变动，本图控制区为 DeepState 2026-08-18 快照；ISW 官方控制评估图已更新至 2026-08-18 13:30 ET。ISW 评估 8 月 17 日俄乌双方均无确认推进；俄方在亚历山德里夫卡/胡里艾伯莱方向的占领声称（含大量 AI 篡改视频）持续被 ISW 评估为「不可信」。' +
